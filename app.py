...
def render_customer_analysis(customer_details: Dict):
    ...
    st.markdown("""<div style="padding: 20px; border-radius: 10px; border: 1px dashed #FFCBA4; background: #FFFDF7;">""", unsafe_allow_html=True)
    for title, key in sections:
...
def render_app():
    ...
    if st.session_state.last_result:
        ...
        st.markdown("### 6. 🍀 고객용 리포트 배포 (다운로드 권장)")
        
        final_html_with_script = generate_report_html(st.session_state.report_package_data)
        
        st.download_button(
            label="📥 리포트 HTML 파일 다운로드 (가장 안정적)",
            data=final_html_with_script.encode('utf-8'),
            file_name=f"{name}_희구소_2026_리포트.html",
            mime="text/html",
